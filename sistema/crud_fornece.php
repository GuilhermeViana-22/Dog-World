...
if(isset($_REQUEST["cod_fornecedor"])) {
    if(isset($_REQUEST["excluir"])) {
        ...
        $consulta = "UPDATE fornecedor SET cnpj = '".$_REQUEST["cnpj"]."', fornecedor = '".$_REQUEST["fornecedor"]."', logradouro = '".$_REQUEST["logradouro"]."',cep = '".$_REQUEST["cep"]."', num_comp = '".$_REQUEST["num_comp"]."', cidade = '".$_REQUEST["cidade"]. "', estado = '".$_REQUEST["estado"]."', telefone = '".$_REQUEST["telefone"]."' , ativo = '".$_REQUEST["ativo"]."' WHERE cod_fornecedor = ".$_REQUEST["cod_fornecedor"]."";
                
        $operacao = 2;
    }
} else {
    $consulta = "INSERT INTO fornecedor ( cnpj, fornecedor, logradouro, cep, num_comp, cidade, estado, telefone, ativo) VALUES ('".$_REQUEST["cnpj"]."','".$_REQUEST["fornecedor"]."','".$_REQUEST["logradouro"]."', '".$_REQUEST["cep"]."','".$_REQUEST["num_comp"]."','".$_REQUEST["cidade"]."','".$_REQUEST["estado"]."','".$_REQUEST["telefone"]."' ,'".$_REQUEST["ativo"]."' )";
